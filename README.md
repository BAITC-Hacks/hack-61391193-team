...
Это интерактивный тренажёр городских решений. Пользователь распределяет ограниченный виртуальный бюджет между пятью инициативами и видит, как его выбор меняет показатели районов Астаны, кому помогает больше всего и какие проблемы остаются нерешёнными.

## Для кого и зачем

Городскому аналитику или управленцу приходится сопоставлять транспорт, озеленение, социальную инфраструктуру, безопасность и городские сервисы в одном бюджете. Жителям и участникам обсуждений трудно понять, почему нельзя одновременно профинансировать всё и как решение для одного района связано с положением других. Симулятор делает эти компромиссы видимыми и обсуждаемыми **до принятия реальных решений**.

Здесь можно выбрать ровно пять мероприятий из каталога, сравнить результат со стартовым состоянием и с лучшим допустимым набором, увидеть изменения на карте и получить объяснение на русском языке. При оценке учитываются средние показатели города, самый слабый район и критические значения отдельных показателей. AI объясняет уже рассчитанные результаты; бюджет, ограничения и **Astana Quality of Life Score** вычисляются детерминированно.

**Граница применимости:** показатели, стоимость и эффекты мероприятий взяты из синтетического конкурсного датасета. Продукт сейчас подходит для обучения, обсуждения приоритетов и сравнения сценариев в заданной модели. Он не предсказывает фактические последствия строительства или инвестиций в Астане. Геоданные Overture используются как пространственный контекст, а не как замена официальной статистике.

## Что показать на демо

1. Выбрать пять мер при бюджете 100: backend проверяет цену, повторы, районы и конфликты.
2. Увидеть Score, изменения показателей по районам, оставшиеся критические проблемы и карту объектов.
3. В ответе API сравнить свой набор с рассчитанным максимумом **этой модели** и прочитать объяснение сильных сторон и компромиссов. При недоступности внешнего AI работает объяснение по шаблону.

Проверочный пример из датасета стоит **95** единиц и на модели шести районов повышает Score с **52.33242049** до **56.31781049**. Готовый запрос: [`docs1/simulation-example.json`](docs1/simulation-example.json).

## Быстрый запуск

Нужен Docker с Compose. Из корня репозитория запустите backend и PostgreSQL:

```bash
docker compose --env-file .env.example up -d --build backend-akim
```

После старта откройте [Swagger UI](http://localhost:8081/swagger-ui/index.html) или проверьте пример из корня репозитория:

```bash
curl -X POST http://localhost:8081/api/v1/simulation/calculate \
  -H 'Content-Type: application/json' \
  --data-binary @docs1/simulation-example.json
```

Контейнерный API доступен на `http://localhost:8081/api/v1`. `compose.yaml` содержит только `backend-akim` и необходимый ему `postgres`. Без ключа внешней модели расчёт и шаблонное объяснение работают.

**Интерфейс:** Next.js frontend сейчас находится в ветке [`front-end`](https://github.com/BAITC-Hacks/hack-61391193-team/tree/front-end/frontend) и ещё не объединён с `main`. Чтобы запустить его рядом с backend из текущего клона:

```bash
git fetch origin front-end
git worktree add ../akim-frontend origin/front-end
cd ../akim-frontend/frontend
npm ci
BACKEND_ORIGIN=http://127.0.0.1:8081 npm run dev
```

Откройте [http://localhost:3000](http://localhost:3000). Если папка `frontend/` уже появилась в вашей ветке, начните с `cd frontend` и выполните `npm ci` и команду запуска. Для PowerShell переменная задаётся отдельно: `$env:BACKEND_ORIGIN='http://127.0.0.1:8081'`, затем `npm run dev`. Frontend также содержит [свои инструкции](https://github.com/BAITC-Hacks/hack-61391193-team/blob/front-end/frontend/README.md). Для работы интерфейса нужны Node.js и npm; версии зависимостей зафиксированы в `frontend/package-lock.json`.

**AI-объяснение через API:** скопируйте `.env.example` в `.env`, задайте в нём `OPENAI_API_KEY` и запустите `docker compose up -d --build backend-akim`. По умолчанию используются `https://api.openai.com/v1/chat/completions` и модель `gpt-4.1-mini-2025-04-14`; их можно переопределить через `SIMULATION_LLM_URL` и `SIMULATION_LLM_MODEL`. Docker Compose передаёт ключ backend без вывода во frontend. Без Docker: `scripts/run-backend.sh` (берёт ключ из `.env`, без PostgreSQL), проверка — `EXPECT_SOURCE=llm scripts/smoke-simulation.sh`. Наличие ключа само по себе не подтверждает доступ к конкретной модели; при ошибке API возвращается шаблонное объяснение. Значения ключей не добавляйте в Git. Подробный запуск, авторизация и все маршруты описаны ниже и в [документации backend](Readme-backend.md).

## Команда

| Участник | Роль | Подтверждение в репозитории |
|---|---|---|
| [Arsen](https://github.com/math-lover47) | AI / ML | Автор первоначального Python-стека и пайплайна Laya. |
| [Zhunis Ayanat](https://github.com/barcek2281) | Backend | Автор Java API, расчёта и документации backend. |
| [Rask1lll](https://github.com/Rask1lll) | Frontend | Автор интерфейса в ветке `front-end`; отображаемое полное имя не указано в истории коммитов. |

Перечислены люди, подтверждённые коммитами. Технический бот, создавший репозиторий, в состав команды не включён.
...
После запуска откройте **[Swagger UI для Docker](http://localhost:8081/swagger-ui/index.html)** или [для локального Java](http://localhost:8080/swagger-ui/index.html). В разделе `Simulation` выберите `POST /api/simulation/calculate`, нажмите **Try it out → Execute**. Пример из конкурсного датасета уже заполнен: 5 мероприятий, бюджет 95, на модели шести районов итог **56.31781049** (для интерфейса **56.32**).
...
```bash
curl http://localhost:8081/v3/api-docs.yaml --output docs1/openapi.yaml
```
...
```bash
curl -X POST http://localhost:8081/api/simulation/calculate -H "Content-Type: application/json" --data-binary @docs1/simulation-example.json
```
...
5 решений пользователя → валидация → точный расчёт пользователя
  ...
  → сводка фактов → LLM → проверка чисел и мер (≤ 1 исправление) → итоговое объяснение
```
...
Текущий максимум модели `v2-saraishyk` с каталогом из 16 мер — **57.01147549**, бюджет **98**: M2 и M14 для города, M3, M8 и M9 в Нуре. Проверено **4 859 952** допустимых варианта; разница с исходным примером — **0.693665**. Расширение каталога сохранило прежний лучший набор. Эти сведения возвращаются в `bestSolution` и `comparison`. Готовый запрос: [`docs1/simulation-optimal-example.json`](docs1/simulation-optimal-example.json); для повторения максимума также можно передать `bestSolution.decisions` в следующий POST.

Для подключения достаточно `OPENAI_API_KEY` в `.env`. Остальные переменные backend необязательны (значения по умолчанию подставляют Compose и `scripts/run-backend.sh`):

```env
OPENAI_API_KEY=
SIMULATION_LLM_URL=https://api.openai.com/v1/chat/completions   # любой OpenAI-совместимый chat-completions
SIMULATION_LLM_MODEL=gpt-4.1-mini-2025-04-14
SIMULATION_LLM_API_KEY=                                         # пусто → OPENAI_API_KEY
SIMULATION_LLM_TIMEOUT_MS=10000
```

Как работает AI-объяснение:

1. **Сводка фактов** (`SimulationLlmFacts`, ≈5 КБ). Модель получает не весь ответ, а округлённые до 2 знаков данные: меры обоих планов с районом, стоимостью, лагом и приростом балла района; Score, бюджет, слабейший район, показатели ниже 40, изменения районов, крупнейшие приросты, все снижения с мерой-причиной; синергии; какие меры есть только у пользователя или только в лучшем плане, их суммарную стоимость и разницу по слабейшему району. Если план пользователя уже оптимален, лучший план не передаётся.
2. **Проверка ответа** (`SimulationLlmTextGuard`). Каждое число в тексте должно быть в сводке (допускается округление), каждая мера `M1`–`M14` — в сравниваемых планах; текст — русская проза без JSON.
3. **Одно исправление.** Если проверка не пройдена, модели возвращается конкретная причина (например, `number 2.97 is not in the facts`) и она переписывает ответ один раз. Если и второй ответ не прошёл — объяснение по шаблону.

Модель меняет только `explanation.summary`; `strengths`, `risks`, `recommendations`, Score, бюджет и все показатели считает backend. `explanation.source` = `llm` только для принятого ответа. При ошибке API, таймауте или отказе проверки возвращаются те же числа и шаблон; причина пишется в лог без ключа и текста запроса. Проверено на `gpt-4.1-mini-2025-04-14`: 5 из 5 тестовых сценариев приняты (один после исправления), 3–6 с на ответ.
...
| `GET` | `/api/v1/measures` | Каталог из 16 мероприятий |
| `GET` | `/api/v1/districts/{districtId}/measures` | Все 16 мероприятий в контексте выбранного района, например `nura` |
...
Команды ниже используют локальный Java backend на порту `8080`. Для Docker замените порт на `8081`.
...
Получить 16 вариантов мероприятий для Нуры:
...
Каталог расширен до 16 мероприятий: районные **M15 «Приоритетная уборка снега и наледи»** и **M16 «Строительство и очистка ливневой канализации»** входят в направление `services` («Сервисы», M12–M16). Их параметры синтезированы для игры: стоимость 12 / 22 единицы, лаг 1 / 3 квартала. M15 улучшает T1, T2 и B2; M16 — C1, T1 и B2. Десять показателей, их веса, районный датасет, формула Score и ограничения выбора сохранены, `modelVersion` остаётся `v2-saraishyk`. [Параметры и обоснование](docs1/municipal-measures.md). [Готовый запрос с обеими новыми мерами в Нуре](docs1/simulation-municipal-example.json) содержит пять решений общей стоимостью 85.
